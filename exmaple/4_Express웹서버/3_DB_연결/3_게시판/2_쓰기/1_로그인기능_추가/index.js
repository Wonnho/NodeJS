...
// 게시판 메인 페이지 렌더링 - 메인 동적 페이지가 바뀔 것을 대비해서 이름을 / 에서 /boardMain으로 변경
...
app.get('/detailPost/:id', async (req, res) => {
    ...
    try {
        ...
        const postResult = await conn.execute(
            `SELECT p.id, p.title, u.username AS author, p.content, TO_CHAR(p.created_at, 'YYYY-MM-DD') AS created_at, p.views, p.likes 
            FROM posts p
            JOIN users u ON p.author_id = u.id
            WHERE p.id = :id`,
            [postId],
